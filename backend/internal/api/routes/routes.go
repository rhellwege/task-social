...
func SetupServicesAndRoutes(app *fiber.App, querier repository.Querier) {
	...
	api.Get("/club/:club_id", handlers.GetClub(clubService)) //for some reason says its not properly declared? maybe a me issue from needing to revert backend to this timeframe. Update: apparently the club_handler.go was out of date by 60 lines in my local copy in this branch which i started from the club endpoints branch or main at the time i think
